...
import network 
import data_flow
from loss import Cross_penalized
...
class Train():
    def __init__(self, 
        ...
        self.model = network.Dropout_GCN_pool(sizes = layer_size, n_classes = n_classes)  
    ...
    def _train_step(self, data, labels):
        with tf.GradientTape() as tape:
            ...
            An,h1,y_prob,z = self.model(data, training=True)
            
            labels_reshaped = tf.cast(tf.reshape(labels, [-1, FLAGS['num_classes'].value]),tf.float32)
            y_prob_reshaped = tf.cast(tf.reshape(y_prob, [-1, FLAGS['num_classes'].value]),tf.float32)

            
            loss = Cross_penalized(A = An, hidden = h1,z = z)(labels_reshaped, y_prob_reshaped)
        ...
        self.train_auc(labels_reshaped, y_prob_reshaped )
        return y_prob_reshaped
    
            
    @tf.function
    def _test_step(self, data, labels):
        An,h1,y_prob = self.model(data,training = False)
        labels_reshaped = tf.cast(tf.reshape(labels, [-1, FLAGS['num_classes'].value]),tf.float32)
        y_prob_reshaped = tf.cast(tf.reshape(y_prob, [-1, FLAGS['num_classes'].value]),tf.float32)
        preds = tf.argmax(y_prob_reshaped, axis=-1)
        preds = self._convert_prediction_to_one_hot(preds)
        loss = Cross_penalized(A = An, hidden = h1)(labels_reshaped, y_prob_reshaped)
        self.test_loss(loss)
        self.test_auc(labels_reshaped, y_prob_reshaped)
        return y_prob_reshaped, preds
    ...
    def _validation(self,step):
        ...
        for data, labels in self.val_ds:
            y_prob,y_pred = self._test_step(data, labels) 
            y_prob_ls.append(y_prob)
            y_pred_ls.append(y_pred )
            labels_ls.append(labels)
        y_prob_ls = np.array(y_prob_ls).reshape((-1, FLAGS['num_classes'].value))
        ...
        prediction = tf.argmax(y_pred_ls,axis = -1)
        annotation = tf.argmax(labels_ls,axis = -1)
        prediction_prob = y_prob_ls[:,1]
        ...
        auc = network.multiclass_roc_auc_score(labels_ls,y_pred_ls)
    ...
    def evaluate(self, data_dir, subset, checkpoint_dir,return_prob = False):
        ...
        prob,preds,labels_ls,link_loss = self._predict()
        ...
        print('Link prediction loss:',link_loss)
    ...
    def _predict(self):
        ...
        for data, labels in self.test_ds:
            An,h1,prob = self.model(data,training = False)
            ...
            # link_loss += tf.nn.sigmoid_cross_entropy_with_logits(labels=labels, logits=prob[:,1])
            # cnt+=1

            
        # link_loss = link_loss/cnt
        prob_ls = np.array(prob_ls).reshape((-1, FLAGS['num_classes'].value))
        preds_ls = np.array(preds_ls).reshape((-1, FLAGS['num_classes'].value))
        labels_ls = np.array(labels_ls).reshape((-1, FLAGS['num_classes'].value))
        return prob_ls,preds_ls,labels_ls,link_loss
...
if __name__ == '__main__':
    ...
    if option == 1:
        ...
        folder_list = ['CV_only/c0_Learning_rate_0.01_layer_size_16_layer_num_1_Mar_23_22_24_13',
                        'CV_only/c1_Learning_rate_0.01_layer_size_16_layer_num_1_Mar_24_01_19_00',
                        'CV_only/c2_Learning_rate_0.01_layer_size_16_layer_num_1_Mar_24_04_08_14'    ] 
